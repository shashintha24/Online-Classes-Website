...
import { Badge, Icons, OutlineBtn } from "./StudentShared";
...
function iconForType(type) {
  ...
  if (value.includes("FEE")) return { icon: <Icons.CreditCard />, bg: "bg-emerald-100 text-emerald-600" };
  if (value.includes("CONTENT")) return { icon: <Icons.File />, bg: "bg-purple-100 text-purple-600" };
  return { icon: <Icons.Bell />, bg: "bg-amber-100 text-amber-600" };
}
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <span className={`text-sm ${t.textSub}`}>{unreadCount} unread</span>
        <OutlineBtn small t={t} onClick={markAllRead}>Mark all read</OutlineBtn>
      </div>
        ...
        <div key={group.label}>
          ...
          <div className={`${t.card} rounded-xl overflow-hidden`}>
            {group.items.map((item) => (
              <button key={item.id} onClick={() => markRead(item.id)} className="w-full text-left flex gap-3 px-5 py-4 border-b last:border-0 border-inherit">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${item.iconBg}`}>{item.icon}</div>
